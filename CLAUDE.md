...
- **Plan** (`agencies/{agencyId}/plans/{planId}`, [src/types/billing.ts](src/types/billing.ts)) — name + monthly price + a bundle over the existing feature gates (`PLAN_GATE_KEYS`, all agency gates minus the parked Get Leads; `*HiddenWhenDisabled` flags stay manual). Creating a plan creates a Stripe Product + recurring monthly Price; price edits mint a NEW Stripe Price (immutable) and deactivate the old one — existing subscribers keep their signup price; gate edits **re-apply to every active/past_due sub-account on the plan**. Archive = no new assignments, live subscriptions untouched.
- **Per-sub-account state** (`SubAccountDoc.billing`, type `SubAccountBilling`) — `status: comped | pending | active | past_due | canceled`, plan denorms (`planName`/`priceCents`/`currency`), `specialPriceCents` (per-client override → one-off Stripe Price on the plan's product), Stripe customer/subscription ids, `checkoutTokenHash`, `graceUntil`. **Absent field = comped** — the default for every legacy + new sub-account, so nothing changes until a plan is assigned. Server-only writes (subAccounts client writes are already `false` in rules).
...
Each sub-account can hold **up to `MAX_WEBSITES_PER_SUBACCOUNT` (5) by default** website docs at `subAccounts/{id}/website/{siteId}` (see [src/lib/website/limits.ts](src/lib/website/limits.ts)). An agency owner can override this per sub-account via `SubAccountDoc.websiteMaxSites` (null = default, `-1` = unlimited, or a positive integer) — set from the Manage dialog's Website gate row (a numeric input + "Unlimited" checkbox, only shown while the gate is on). `lib/website/limits.ts::effectiveWebsiteCap()` is the single place that resolves the override against the default; the create-site route, the client "Add website" affordance, and the AI Suite's site-slot lookup all call it so none of them can drift. Not wired to Client Billing plans yet, but shaped so a plan could carry the field later as a paid-tier "more/unlimited funnels" lever. The legacy singleton at `.../website/main` remains valid as one of the slots — no migration. The page at [my-ghl-app/src/app/(dashboard)/sa/[subAccountId]/website/page.tsx](src/app/(dashboard)/sa/[subAccountId]/website/page.tsx) owns the collection `onSnapshot` + the "Add website" affordance + the account-wide activation gate, and renders one card per site via [src/components/website/website-builder.tsx](src/components/website/website-builder.tsx). Each `<WebsiteBuilder/>` holds its own form (a long sectioned form mirroring gitpage's typeform inputs — Basics / Pages / Services / Business / Design / FAQ), collapse state, and build/status logic; the parent passes the live doc down (one listener, not one per card). Cap is enforced server-side in `POST /api/sub-accounts/[id]/website` (the create-site route, returns 409 once the effective cap is hit). Site cards expose a **Remove** (trash) action that deletes the doc to free a slot — distinct from **Rebuild** (reset to draft).
...
### Content-fabrication guardrail

gitpage's generic (non-niche) local template fills empty sections with invented content when it has nothing real to work with: fabricated testimonials attributed to fictional people (the same names — e.g. "Sarah Mitchell" — recur verbatim across unrelated sites), inflated stat badges ("450K+ Readers", "4.8★ Rating", "10K+ Lives Transformed"), fake founder/company origin stories, phantom program infrastructure (video modules, coaching calls, a community forum, certification exams) for products that are just a book or a PDF, and guarantee language implying a real refund policy that was never established. None of this comes from our `WebsiteConfig` payload — confirmed against the full field list, there is no testimonials/stats/guarantee input anywhere in it, and no config flag suppresses it. This is a real integrity problem (fabricated social proof, invented credentials), not a style issue — never ship a gitpage build without checking for it.

- **Automatic detection**: [src/lib/website/content-audit.ts](src/lib/website/content-audit.ts) `auditGeneratedContent(html)` scans the live page for known fabrication patterns (regex, no LLM call) across five categories — `fabricated_social_proof`, `fabricated_testimonials`, `fabricated_credentials`, `fabricated_guarantee`, `phantom_program_features`. Wired into the poll route ([.../website/[siteId]/poll/route.ts](src/app/api/sub-accounts/[id]/website/[siteId]/poll/route.ts)): the moment a build reaches `Published`, it fetches the live URL and runs the audit, storing the result on `WebsiteDoc.contentFlags`. A fetch failure never blocks the build from going "ready" — the operator just won't see a banner until the next successful scan. Detection only, deliberately — a regex match can't know what honest replacement text should say, so this **flags for human review, it does not auto-fix**.
- **UI**: [src/components/website/website-builder.tsx](src/components/website/website-builder.tsx) renders a destructive-toned banner on the site card when `contentFlags` is non-empty, listing which categories were detected. Same visual slot as the existing `partialErrors` banner, one tier more severe.
- **The actual fix — gitpage's MCP tools**: gitpage exposes an MCP server at `https://www.gitpage.site/api/mcp` (JSON-RPC 2.0, both SSE-streamed responses) that authenticates with the same `GITPAGE_API_KEY` — no separate OAuth/connector setup needed for server-side calls. Confirmed-working tool sequence for fixing flagged content: `list_sites` (map `formResponseId`/repo name → `pageId`) → `read_site_file` (pageId + `"index.html"`, returns content + sha) → `edit_site_file` (diff-style `old_string`/`new_string` edits, `replace_all` supported, stages a draft — does NOT commit) → `validate_draft` (structural sanity check, catches truncation/malformed HTML before it goes live) → `publish_draft` (commits to git, returns the new live URL). `restore_previous_version` gives one-shot rollback if a publish needs undoing. This is a manual/operator-triggered workflow today (run via a one-off script calling the MCP endpoint), not automated — the fix requires writing honest replacement copy grounded in the product's real description, which is exactly the judgment step that shouldn't be automated without review.
- **When drafting new site copy** (manually or via `create_website`): a real founder/author bio, real specific numbers (years of experience, pricing), and real program features (if any) sourced from the operator's own description survive fine — gitpage only invents filler where a section would otherwise be empty. Keeping `heading`/`hero_statement`/`features`/`benefits` grounded in real product facts doesn't prevent the testimonials/stats sections from being generated, but it does keep the About/FAQ sections it fills in closer to true. Still audit every build regardless.
...
**Checkout session creation** — `POST /api/lp/[funnelId]/checkout/session` (public, unauthenticated — the funnelId + sectionId are the only "credentials," same trust model as `/api/forms/[id]/submit`), rate-limited per-IP (`lib/funnels/checkout-rate-limit.ts`, 20/hour, in-memory). Non-subscription sessions set `payment_intent_data.setup_future_usage: "off_session"` so the card is saved for a later upsell charge (subscriptions already save the card automatically for future invoices, so this is skipped there). **`success_url` routing (corrected 2026-09-16 — the "known gap, not yet fixed" note previously here was stale and described behavior that commit `6bf84ed` had already replaced):** the redirect branches on `CheckoutConfig.upsellFunnelId`. A normal purchase with no upsell configured lands on the dedicated post-checkout confirmation page **`/lp/[funnelId]/thanks?paid=1`**. When the section DOES name an upsell step, the buyer lands directly on that step at `/lp/[upsellFunnelId]?session_id={CHECKOUT_SESSION_ID}` — the `session_id` is there because the upsell page's one-click charge route reads it to find the saved card, so it applies to the upsell path ONLY and is not part of the normal success return. Verified end to end against a real test-mode purchase on 2026-09-16.

**Real gap that remains: there is no post-purchase email of any kind.** Funnel Checkout has no mailer anywhere in its path (`lib/funnels/checkout-webhook.ts` imports none), `receipt_email` is never set on the Checkout Session or the PaymentIntent, and `funnel.order.completed` is consumed only by `emitWebhookEvent` (outbound API subscribers) and `lib/workflows/lifecycle-events.ts` (adds the `purchased` tag). No receipt, no order confirmation, no fulfillment email is sent or attempted. The thank-you page's copy states only what the product actually does and deliberately does not depend on Stripe's account-level "Successful payments" customer-email setting. If a real confirmation email is ever built, that copy changes with it.
...
**Revocation** rides the same endpoint via `action: "revoke"`, sent from the same Stripe webhook branch that revokes the entitlement rows — which Stripe only reaches once the subscription has genuinely ended, so a cancel-at-period-end keeps access for the period already paid for. It fires only when `growth_operations` was among the revoked rows, so a Zeno cancellation never touches Flow. Only `ascendOperations.status` flips; the workspace, its data, the membership, the identity link and the mapping all survive so a resubscribing customer returns to what they had. Two refusals matter: a grant naming a **different** `clerkUserId` is not withdrawn (fail closed), and `revokedAt` is an **ordering guard** — a provisioning call signed *before* the withdrawal cannot resurrect the workspace, so only a request signed after it (meaning Ascend re-checked a live entitlement) reactivates. A replayed revoke is a no-op that preserves the original `revokedAt`, because re-stamping it would push the guard forward and reopen the window.
