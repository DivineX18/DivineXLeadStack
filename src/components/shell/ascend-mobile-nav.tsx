...
export function AscendMobileNav({ branding, navigation, capabilities, zenoHref }: AscendMobileNavProps) {
  ...
  return (
    <>
      <Button variant="ghost" size="icon" className="text-[var(--dx-text-secondary)] hover:bg-[var(--dx-hover)] hover:text-[var(--dx-text-primary)] md:hidden" onClick={() => setOpen(true)} aria-label="Open navigation">
        <Menu className="h-5 w-5" aria-hidden="true" />
      </Button>

      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="left" className="theme-ascend w-72 border-[var(--dx-border-subtle)] bg-[var(--dx-surface-1)] p-4 text-[var(--dx-text-primary)]">
