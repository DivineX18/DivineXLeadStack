...
import { DocumentShowcase, ProcessFlow, SectionShell, SplitLayout } from "./composition";
...
export function BenefitsGridSection({
      ...
      {config.items.map((item, i) => {
        ...
        return (
          <div key={i} className="flex items-start gap-4">
            <span className="mt-0.5 flex h-11 w-11 shrink-0 items-center justify-center rounded-xl" style={badgeStyle}>
              <Icon className="h-5 w-5" />
            </span>
            <div className="min-w-0">
              <h3 className="text-lg font-bold tracking-tight">{item.title}</h3>
              {item.description && <p className="mt-1 leading-relaxed opacity-80">{item.description}</p>}
            </div>
          </div>
        );
      })}
  ...
  // THE PROMISE BEAT, WHERE THE GRID CAN ACTUALLY TAKE ONE.
  //
  // Never by default: the visual story only sets `beatVisual` here when the
  // concept asserts a relationship the cards cannot express, and only on this
  // default variant. Two treatments, chosen upstream by list length — a short
  // list can give up half its width and still read as a list; a longer one
  // takes a visual that LEADS it, stating the change the cards then detail.
  if (config.beatVisual) {
    if (config.beatVisualAnchored) {
      return (
        <section className="px-4" style={{ paddingBlock: "var(--flow-py, 3rem)" }}>
          <div className="mx-auto max-w-4xl">
            {heading}
            <div className="mx-auto mb-10 max-w-2xl">
              <BeatVisual visual={config.beatVisual} accentColor={accentColor} />
            </div>
            <div className="mx-auto max-w-2xl">{checklist}</div>
          </div>
        </section>
      );
    }
