...
export function ProcessFlow({
  ...
  //
  // The track floor used to be a flat 180px, which at the section's own width
  // resolves to five columns — so a six-step process rendered five across and
  // ONE stranded underneath, on both the Summit and Booking pages. A single
  // card alone on a second row reads as a layout failure, and it was produced
  // deterministically by our own grid rather than by anything the model wrote.
  //
  // The floor is therefore chosen from the step COUNT: wide tracks pack fewer
  // per row, so raising the floor for counts that would strand one gives a
  // balanced 3 + 3 instead of 5 + 1. Still `auto-fit`, so the mobile collapse
  // to a single column is unchanged, and `min(100%, …)` keeps a wide track from
  // overflowing a narrow screen.
  const n = steps.length;
  const columns = n <= 4 ? n : n % 3 === 1 ? 4 : 3;
  const trackFloor = columns >= 5 ? 180 : columns === 4 ? 230 : columns === 3 ? 320 : 460;
  ...
  return (
    ...
    <ol
      ...
      style={{ gridTemplateColumns: `repeat(auto-fit, minmax(min(100%, ${trackFloor}px), 1fr))` }}
    >
      {steps.map((step, i) => (
