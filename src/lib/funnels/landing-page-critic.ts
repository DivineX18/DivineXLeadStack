...
import { layoutFamilyOf } from "@/lib/funnels/page-composition";
...
export function describeComposition(sections: FunnelSection[]): string {
  return sections
    .map((s, i) => {
      ...
      // THE SHAPE THE SECTION RENDERS AS. Without it the Critic was asked to
      // judge composition while being told only what each section is ABOUT —
      // structurally incapable of noticing that six different subjects were
      // rendering as the same narrow centred column, which is the single
      // loudest "this was generated" signal on a real page. Same class of gap
      // as the missing item labels and the missing CTA label above: judgment
      // needs the actual thing being judged.
      const family = layoutFamilyOf(s);
      if (family !== "none") bits.unshift(`layout: ${family}`);
...
THIRD, judge the page's SHAPE. Each section is labelled with the layout it renders as: fold (the opening), centered_column (a narrow centred measure), split (text beside media or two comparable columns), alternating (zigzag rows), grid, band (a full-width strip), card (a contained box), table. Read that sequence as a visitor scrolling would. A page whose sections are nearly all "centered_column" is one silhouette repeated, and it reads as generated no matter how good the words are, flag it with category "visual_rhythm" and name which section would carry a different treatment best. Judge the sequence, not a quota: two of a shape in a row is ordinary rhythm, and a genuinely short page (three or four sections) has no rhythm problem to have. Variety that would require a photograph the page does not have is NOT a correction worth making, say nothing rather than asking for an asset.
