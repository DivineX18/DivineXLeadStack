...
## Growth Blueprint reliability fix — PROMOTED TO PRODUCTION (2026-08-09/10)

✅ VERIFIED WORKING, live. A separate, unrelated bug found mid-session (user reported a 90-day roadmap stuck on "Preparing your personalised blueprint..." for 3 days on standalone `ascend.divinex.io`) — two distinct root causes, confirmed against production data before any fix was written:

1. `GET /zeno/reports/:id`'s `gs_` branch hardcoded `blueprint: null` unconditionally, regardless of whether generation had actually succeeded. Confirmed live: the reported scan's blueprint had generated successfully 43 seconds after the scan, 3 days earlier — purely a display bug, not a generation failure.
2. Separately, 18 of 272 total scans had genuinely never generated a blueprint — no retry existed, and the AI SDK clients had no request timeout (10-minute SDK default), so a hung request could silently block the fire-and-forget promise indefinitely.

**Fix**: corrected the `gs_` route to query `zeno_blueprints` (matching the sibling `bp_` branch and public share routes, which were never broken); added a 120s timeout to both Anthropic and OpenAI clients; added a 10-minute-capped poll to both report pages (the authenticated page previously never polled at all; the public page polled forever with no cap). Backfilled all 18 affected scans by re-running the exact existing generation function against production — 18/18 succeeded, confirmed 0 remain.

**Branch**: `fix-blueprint-generation-reliability`, commit `290932f`, Ascend BI only.

**Smoke-tested on a temporary staging deployment before promotion** (per standing rule — see memory `feedback_smoke_test_before_promote`): repointed a disposable Render service to this branch, deployed for real, and hit the actual live authenticated route over HTTP (using this codebase's own staging-only dev-auth bypass, `X-Dev-Clerk-User-Id`, since no browser-automation tool was available and Clerk session-minting via API wasn't pursued) — confirmed both the originally-reported scan and a backfilled one returned fully populated blueprints. Two of my own mistakes surfaced and were caught during this process: an env-var `PUT` call that briefly wiped the staging service's config (restored from Flow staging's copy of the shared secret + a fresh pull from production), and a `bash until` loop with inverted conditional logic that falsely reported "deploy live" without actually waiting (caught by checking the deploy list directly, fixed, re-verified).

**Promoted to production**: merged to Ascend BI `main`, deploy `dep-d9sl13ht0dsc73brupi0` confirmed live via Render's own API (commit `290932f`, health check passing) at 2026-08-10T04:17:46Z. User independently confirmed a previously-stuck report now populates on the real `ascend.divinex.io` domain.

**Not addressed in this pass**: a pre-existing, unrelated Resend email-validation error on 2 of the 18 backfilled scans (malformed placeholder "to" address from old prospecting-sourced leads) — affects only the completion notification email, not the blueprint itself.
